...
function fixVanGough (arr){
  for (let i = 0; i < arr.length; i++){
    if (arr[i].name === "Vincent van Dough"){
      artists[i].name = "Vincent van Gough"
    };
  };
  return arr;
};
...
function get20s(arr){
  const filteredList = [];
  for (let i = 0; i < arr.length; i++){
    if (arr[i].years.slice(0,4) > 1899 && arr[i].years.slice(-4) < 2001){
      if (arr[i].years.slice(-4) < 2001){
        filteredList.push(arr[i].name);
      };
  };
}
return filteredList;
...
addArtist(artists, 20, "Raya Schroeder", "1995 - 2020", "Web Design", "American", "Raya is a highly sough-after web designer! She has created websites for NASA and the UN.");
